  ...
  class Preprocessor
    ...
    def run(tip)
      begin
        ...
        extract
        persist!
        Tipsy.log.info "Persisted preprocessed content of Tip(#{@tip.fid})"
      rescue => e
        Tipsy.log.unknown "Unknown exception when analyzing Tip(#{@tip.fid}):"
        Tipsy.log.unknown e
      end
    end
    ...
    def annotated_content
      @annotated ||= StanfordCoreNLP::Annotation.new(@tip.content)
    end
    ...
    def extract
      ...
      @content_lemma = annotated_content.get(:tokens).map(&:lemma)
      @content_tokens = annotated_content.get(:tokens).map(&:value)
      @content_tags = annotated_content.get(:tokens).map(&:tag)
      @emoticons = @tip.content.scan emoticons_regex
    end

    def persist!
      @tip.content_tokens = @content_tokens
      @tip.content_lemma = @content_lemma
      @tip.content_tags = @content_tags
      @tip.emoticons = @emoticons
      @tip.save!
    end
